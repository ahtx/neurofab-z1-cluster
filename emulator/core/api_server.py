...
class Z1APIServer:
    ...
    def __init__(self, cluster: Cluster, host: str = '127.0.0.1', port: int = 8000):
        ...
        # SNN coordinator
        self.snn_coordinator = ClusterSNNCoordinator(cluster)
    ...
    def _setup_routes(self):
        ...
        def inject_spikes():
            ...
            for spike_data in spikes_data:
                ...
                # Find which node has this neuron
                # For simplicity, assume neuron_id maps to node 0
                engine = self.snn_coordinator.get_engine(0, 0)
                if engine:
                    spike = Spike(
                        neuron_id=neuron_id,
                        source_node=0,
